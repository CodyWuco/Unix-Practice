...
void get_stats(driver_t driver_list[NRACERS], stats_t *race_stats)
{
    ...
    for( i = 0; i < NRACERS; i++){
        ...
        for ( j = 0; j < TRIES; j++){
            if(driver_list[i].tries[j] > race_stats->slowest_time){
                race_stats->slowest_time = driver_list[i].tries[j];
            }
            if(driver_list[i].tries[j] < driver_list[i].best_time){
                driver_list[i].best_time = driver_list[i].tries[j];
            }
        }
        best_time_total += driver_list[i].best_time; 

        if(race_stats->fastest_time > driver_list[i].best_time){
            race_stats->fastest_time = driver_list[i].best_time;
        }
    }
       
    race_stats->average_best_times = (best_time_total / NRACERS);

    for( i = 0; i < NRACERS; i++){
        driver_list[i].deviation = ( race_stats->fastest_time -                                         driver_list[i].best_time);
    }
